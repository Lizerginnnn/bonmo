...
function parseRegistration_(e) {
  ...
  const meetings = Array.isArray(data.meetings) ? data.meetings.map(String) : [];
  const receipt = data.receipt;

  if (!name || !telegram || !meetings.length) throw new Error('Не заполнены обязательные поля');
  if (!receipt || !receipt.data) throw new Error('Нет чека');

  return { name, telegram, meetings, receipt };
}

function decodeReceipt_(receipt) {
...
function saveToSpreadsheet_({ name, telegram, meetings }, receipt) {
  ...
  if (!spreadsheetId || !folderId) throw new Error('Таблица не настроена: запустите setupSpreadsheet');

  const now = new Date();
  const fileName = `${Utilities.formatDate(now, TIME_ZONE, 'yyyy-MM-dd HH-mm')} ${name} — ${receipt.getName()}`;
  const file = DriveApp.getFolderById(folderId).createFile(receipt.copyBlob().setName(fileName));
...
function sendToTelegram_(token, chatId, registration, receipt) {
  ...
  if (text.length <= CAPTION_LIMIT) {
    ...
    callTelegram_(token, 'sendMessage', { chat_id: chatId, text: text, parse_mode: 'HTML' });
    callTelegram_(token, 'sendDocument', { chat_id: chatId, document: receipt });
  }
}
...
function setupSpreadsheet() {
  ...
  const spreadsheetId = props.getProperty('SPREADSHEET_ID');
  const spreadsheet = spreadsheetId ? SpreadsheetApp.openById(spreadsheetId) : SpreadsheetApp.create('бонмо — заявки');
  if (!spreadsheetId) {
  ...
  if (!spreadsheet.getSheetByName(SHEET_NAME)) spreadsheet.insertSheet(SHEET_NAME);
...
function showChatIds() {
  ...
  if (!token) throw new Error('Сначала задайте TELEGRAM_BOT_TOKEN в Script properties');
  ...
  updates.forEach((update) => {
    ...
    if (chat) chats[chat.id] = chat.title || [chat.first_name, chat.last_name].filter(Boolean).join(' ') || chat.username;
  });

  const ids = Object.keys(chats);
  if (!ids.length) {
    console.log('Чатов не найдено. Напишите боту /start (или добавьте его в группу и напишите там что-нибудь) и запустите снова.');
    return;
  }
...
function sendTestMessage() {
  const { token, chatId } = getTelegramSettings_();
  callTelegram_(token, 'sendMessage', { chat_id: chatId, text: 'Проверка: бот формы бонмо́ подключён ♥' });
